 ...
 *                                                                           *
 *   $Revision: 739 $                                                         *
 *   $Date: 2012-10-08 11:49:14 +0200 (Mo, 08 Okt 2012) $                   *
 *                                                                           *
\*===========================================================================*/
...
read(std::istream& _in, BaseImporter& _bi, Options& _opt)
{
  ...
  while( _in && !_in.eof() )
  {
    ...
    else if (keyWrd == "v")
    {
      ...
      if ( !stream.fail() )
      {
        vertexHandles.push_back(_bi.add_vertex(Vec3f(x,y,z)));
        ...
        if ( !stream.fail() )
        {
          if (  userOptions.vertex_has_color() ) {
            ...
            colors.push_back(Vec3uc((unsigned char)r,(unsigned char)g,(unsigned char)b));
          }
        }
      }
    }
